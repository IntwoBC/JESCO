report 50107 "Aged Accounts Receivable IT"
{
    DefaultLayout = RDLC;
    RDLCLayout = '.\Layout Reports Rdls\AgedAccountsReceivable.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Receivable IT';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            column(TotalAdditionalCurrencyAmt; TotalAdditionalCurrencyAmt)
            {
                AutoFormatType = 1;
            }
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(TempCustLedgEntryLoop; "Integer")
                    {
                        ...
                        column(AdditionalCurrencyRemAmt; AdditionalCurrencyRemAmt)
                        {
                            AutoFormatType = 1;
                        }
                        ...
                        column(AdditionalCurrencyAmt; AdditionalCurrencyAmt)
                        {
                            AutoFormatType = 1;
                        }
                        ...
                        begin
                            ...
                            Clear(AdditionalCurrencyRemAmt);
                            /* Clear(ExchangeRate);
                            if AdditionalCurrency <> '' then begin
                                Clear(CurrencyExchangeRate);
                                //     ExchangeRate := CurrencyExchangeRate.GetCurrentCurrencyFactor(AdditionalCurrency);
                                //     AdditionalBalanceCaption := StrSubstNo('Balance (%1)', AdditionalCurrency);
                                // end else begin
                                //     ExchangeRate := 1;
                                //     AdditionalBalanceCaption := StrSubstNo('Balance (%1)', GLSetup."LCY Code");
                                AdditionalBalanceCaption := StrSubstNo('Balance (%1)', AdditionalCurrency);
                                if CurrencyExchangeRate.Get(AdditionalCurrency, CustLedgEntryEndingDate."Posting Date") then
                                    ExchangeRate := CurrencyExchangeRate."Relational Exch. Rate Amount"
                                else
                                    ExchangeRate := CurrencyExchangeRate.GetCurrentCurrencyFactor(AdditionalCurrency);
                            end else begin
                                ExchangeRate := 1;
                            ...
                            if AdditionalCurrency <> '' then begin
                                ...
                                AdditionalCurrencyRemAmt := CurrencyExchangeRate.ExchangeAmount(CustLedgEntryEndingDate."Remaining Amt. (LCY)", GLSetup."LCY Code", GLSetup."Additional Reporting Currency", CustLedgEntryEndingDate."Posting Date");
                                AdditionalCurrencyAmt += CurrencyExchangeRate.ExchangeAmount(CustLedgEntryEndingDate."Remaining Amt. (LCY)", GLSetup."LCY Code", GLSetup."Additional Reporting Currency", CustLedgEntryEndingDate."Posting Date");
                                TotalAdditionalCurrencyAmt += CurrencyExchangeRate.ExchangeAmount(CustLedgEntryEndingDate."Remaining Amt. (LCY)", GLSetup."LCY Code", GLSetup."Additional Reporting Currency", CustLedgEntryEndingDate."Posting Date");

                            end else begin
                                AdditionalBalanceCaption := StrSubstNo('Balance (%1)', GLSetup."LCY Code");
                                AdditionalCurrencyRemAmt := CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                                AdditionalCurrencyAmt += CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                                TotalAdditionalCurrencyAmt += CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                            end;
                        end;
            ...
            dataitem(CurrencyTotals; "Integer")
            {
                ...
                column(AgedCLE6RemAmt; AgedCustLedgEntry[5]."Remaining Amount")
                {
                    AutoFormatExpression = CurrencyCode;
                    AutoFormatType = 1;
                }
                    ...
                    if TempCurrencyAmount.FindSet(false, false) then
                        repeat
                            if TempCurrencyAmount.Date <> DMY2Date(31, 12, 9999) then
                                ...
                                AgedCustLedgEntry[5]."Remaining Amount" := TempCurrencyAmount.Amount;
    ...
    var
        ...
        TotalCustLedgEntry: array[4] of Record "Cust. Ledger Entry";
        GrandTotalCustLedgEntry: array[4] of Record "Cust. Ledger Entry";
        AgedCustLedgEntry: array[5] of Record "Cust. Ledger Entry";
        ...
        AdditionalCurrencyRemAmt, AdditionalCurrencyAmt, TotalAdditionalCurrencyAmt : Decimal;
    ...
    begin
        ...
        //PeriodStartDate[4] := CalcDate('-185D', PeriodEndDate[2] + 1);
